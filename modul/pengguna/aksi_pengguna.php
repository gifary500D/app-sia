<?php
 ...
 if($_SERVER["REQUEST_METHOD"]=="POST"){
    if($_GET['act']=="insert"){
        ...
        $query = "insert into pengguna (username, password, nama_lengkap, 
jabatan, email, hak_akses) values ('$username', '$password', '$nama_lengkap',
 '$jabatan', '$email', '$hak_akses')";
        ...
        $hak_akses = $_POST['hak_akses'];
        if(empty($password)){
            $query = "update pengguna set nama_lengkap = '$nama_lengkap', 
jabatan = '$jabatan', email = '$email', hak_akses = '$hak_akses' where 
username = '$username'";
        }else{
            $password = password_hash($password, PASSWORD_BCRYPT);
            $query = "update pengguna set password = '$password', nama_lengkap
